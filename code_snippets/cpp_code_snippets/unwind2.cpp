// unwind2.cpp: Illustrates that raw pointers do not have destructors; only objects do.
#include <iostream>
#include <stdexcept>
using namespace std;
...
void g() {
    ...
    delete pf;   // Unreachable; but delete pf would have called ~Foo() which runs delete p
}
void f() {
    Foo* pf = new Foo(1);
    g();
